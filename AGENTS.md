...
| Browser Use | `src/main/browser-use.ts`, `mcp/browser-tool.ts`, `src/shared/browser-use.ts`, `src/renderer/{browser-panel,browser-tab-strip}.ts`: isolated ordinary-web session, consent, snapshots, model input and right-side native panel. It has no Internal Chromium, bridge, recorder or companion-extension authority. |
...
bridge, or import the Internal Chromium subsystem. `internal-browser.ts` and Browser Use remain
independent owners with different partitions and no cross-imports.
Model-facing routing preserves that boundary explicitly: Browser Use or its Browser panel uses
only Core's `browser` tool. Existing companion-exposed tabs, including hosted ChatGPT pages, use
Desktop `browser_*`; a missing companion-browser result says nothing about Browser Use. Ordinary
website work defaults to Browser Use. Descriptions and refusal guidance may name the other
surface but never dispatch to it or create an Internal Chromium dependency.
...
navigation authority. The app-owned ChatGPT dock remains an independent shell split.
...
a renderer dropdown. Its isolated preload accepts only Browser, Pets, Sidebar and zoom commands.
Every action glyph and visible state check uses the shared Phosphor map from `icons.css`; only the
ChatGPT brand mark is bespoke, and Browser remains the first Internal-only row.
A hidden renderer prewarms after the shell loads, so the first press only snapshots, attaches and
reveals it. Each opening snapshot restarts the composer's short surface reveal, with a reduced-motion
path. The shell admits one toggle request at a time while main remains the open-state owner.
...
Shell panels preserve their spatial origin during routine toggles: Chats and the app-owned
ChatGPT browser reveal from the left, Files and Sub-agents reveal from the right, and Terminal
reveals upward. Stable grid tracks animate the workspace reflow instead of abruptly replacing
layout. Closing the browser still parks its native `WebContentsView` before its track contracts;
reduced motion keeps all state changes immediate. Resizing never inherits toggle transitions.
