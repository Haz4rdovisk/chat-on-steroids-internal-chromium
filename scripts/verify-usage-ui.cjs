...
app.whenReady().then(async () => {
  ...
  await win.webContents.executeJavaScript(`(() => {
    ...
    document.getElementById('usageDays').innerHTML = Array.from({ length: 2 }, (_, table) => '<table class="usage-table"><tr><th>' + (table ? 'Day' : 'Recorded model / effort') + '</th><th>Estimated tokens</th><th>Estimated equivalent</th></tr>' + Array.from({ length: 4 }, (_, row) => '<tr><td>' + (table ? '2026-09-' + (18 - row) : 'gpt-5.6-sol · high') + '</td><td>1.295.179.190</td><td>US$ 621,69</td></tr>').join('') + '</table>').join('');
  ...
  const inspect = async width => {
    ...
    return win.webContents.executeJavaScript(`(() => {
      ...
      const tables = document.querySelector('.usage-table-stack');
      return {
        ...
        summary: rect('.usage-summary'), sections: [...document.querySelectorAll('.usage-section')].map(node => ({
          ...
          surface: { left: node.querySelector('.settings-surface').getBoundingClientRect().left, right: node.querySelector('.settings-surface').getBoundingClientRect().right }
        })),
        ...
        tablesScrolls: tables.scrollWidth > tables.clientWidth,
  ...
  for (const width of [1400, 900]) {
    ...
    assert.equal(result.tablesScrolls, width === 900, JSON.stringify(result));
  ...
  const expanded = await win.webContents.executeJavaScript(`(() => {
    document.getElementById('usageFormulaDetails').open = true;
    const panel = document.querySelector('[data-panel="usage"]');
  ...
  await win.webContents.executeJavaScript(`document.getElementById('usageFormulaDetails').open = false`);
  await inspect(1400);
