// Isolated Chromium render of the production Internal Chromium View-menu renderer.
...
app.whenReady().then(async () => {
  ...
  const fixture = `
    ...
    window.fixtureApply=(theme)=>listener({
      browserOpen:true,petVisible:true,petReady:true,sidebarCollapsed:false,zoomPercent:117,theme,language:'en',
      labels:{browser:'ChatGPT browser',pet:'Desktop pets',sidebar:'Toggle Sidebar',zoomIn:'Zoom In',zoomOut:'Zoom Out',actualSize:'Actual Size'}
    });
  ...
  try {
    ...
    const metrics = await js(`(() => {
      ...
      return {
        ...
        actionSvgs:document.querySelectorAll('.view-menu-surface svg').length,
        iconFont:getComputedStyle(document.querySelector('#viewPet .ico')).fontFamily,
    ...
    assert.deepEqual(metrics.commands, ['browser', 'pet', 'sidebar', 'zoom-in', 'zoom-out', 'zoom-reset']);
    assert.ok(metrics.labels[0].startsWith('ChatGPT browser'), JSON.stringify(metrics));
    assert.ok(metrics.labels[1].startsWith('Desktop pets'), JSON.stringify(metrics));
    assert.deepEqual(metrics.icons, [
      'ico chatgpt-mark', 'ico ph ph-paw-print', 'ico ph ph-sidebar-simple',
      'ico ph ph-magnifying-glass-plus', 'ico ph ph-magnifying-glass-minus', 'ico ph ph-corners-out'
    ]);
    assert.equal(metrics.checks, 3);
    assert.equal(metrics.actionSvgs, 1);
    ...
    assert.deepEqual(metrics.checked, ['browser', 'pet', 'sidebar']);
    ...
    await js(`document.getElementById('viewBrowser').click();document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}))`);
    assert.deepEqual(await js('window.fixtureCommands'), ['browser', 'close']);
    console.log('Internal View menu: six commands, ChatGPT mark, Phosphor action icons, checked states, themes and bounds verified in Chromium.');
  } finally {
    win?.destroy();
    await server.close();
    app.quit();
