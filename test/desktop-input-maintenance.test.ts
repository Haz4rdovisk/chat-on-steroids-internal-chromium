...
it.each(['generating', 'draft', 'attachments', 'input_busy', 'composer_hidden'])('explicit refresh uses one helper without touching a %s user tab', async reason => {
  ...
  expect(h.sendMessage.mock.calls.filter(([id, message]) => id === 8 &&
    !['clf-model-catalog-state', 'clf-recorder-ping'].includes(message.type))).toHaveLength(0);
...
async function worker(inputs: Array<{ id: string; conversationId: string | null; directTurn?: { id: string; startedAt: number }; supersededConversationId?: string }>, modelCatalogRequest?: { nonce: string; expiresAt: number }, priorLocal: Record<string, unknown> = {}, priorSession: Record<string, unknown> = {}, rendered?: Set<number>) {
  ...
  const appFetch = vi.fn(async (_input: string, _init?: RequestInit): Promise<{ ok: boolean; status: number; json: () => Promise<unknown> }> => ({
    ok: true, status: 200,
    json: async () => new URL(_input).pathname === '/hello'
      ? { app: 'chat-on-steroids', bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
  ...
  const fetch = async (input: string, init?: RequestInit): Promise<{ ok: boolean; status: number; json: () => Promise<unknown> }> => {
    ...
    if (path === '/browser-host') {
      ...
      if (body.action === 'reload') { await reload(body.tabId); return { ok: true, status: 200, json: async () => ({ ok: true }) }; }
  ...
  vm.runInContext(`${source}\nglobalThis.testMaintenance = { load, maintain, releaseTab, serializeTab, noteTabConversation, createChatTab, authorizeDocument, ackDesktopInput, drainCommandAcks, inspectRequestedModels, handleBrowserTabUpdated, desktopInput: HANDLERS.desktop_input, catalog: HANDLERS.model_catalog, events: HANDLERS.events, bind: HANDLERS.bind, correlate: HANDLERS.correlate, applyRequestedBrowserPreferences };`, context);
  const api = context.testMaintenance as { releaseTab(...args: any[]): Promise<any>; serializeTab(tab: number, operation: () => Promise<any>): Promise<any>; noteTabConversation(source: any, conversationId: string): Promise<any>; handleBrowserTabUpdated(tab: number, change: object, current?: Tab): Promise<void>; applyRequestedBrowserPreferences(request: object): Promise<void>; authorizeDocument(sender: unknown, message: unknown): Promise<any>; catalog(message: unknown, sender: unknown, source: unknown): Promise<any>; load(): Promise<void>; maintain(): Promise<void>; createChatTab(url: string, background: boolean): Promise<Tab> };
  await api.load();
  Object.assign(api, { query });
  const updated = (id: number, change: object) => { void api.handleBrowserTabUpdated(id, change, tabs.find(tab => tab.id === id)); };
  vm.runInContext('Object.assign(testMaintenance, { offerStopTurns, noteTabConversation, ackCommand })', context);
  return { ...api, scripting, updated, update, inspectModels: (context.testMaintenance as any).inspectRequestedModels as (request: unknown, background: boolean) => Promise<void>, ackDesktopInput: (context.testMaintenance as any).ackDesktopInput as (...args: string[]) => Promise<any>, drainCommandAcks: (context.testMaintenance as any).drainCommandAcks as () => Promise<any>, desktopInput: (context.testMaintenance as any).desktopInput as (...args: any[]) => Promise<any>, events: (context.testMaintenance as any).events as (message: any, sender: any, source: any) => Promise<any>, bind: (context.testMaintenance as any).bind as (message: any, sender: any, source: any) => Promise<any>, correlate: (context.testMaintenance as any).correlate as (message: any, sender: any, source: any) => Promise<any>, create, sendMessage, tabs, fetch: appFetch, windows, remove, reload, local, localSaved, saved };
}
...
describe('one browser maintenance flight per desktop outbox publication', () => {
  ...
  it('keeps a background input in an inactive internal tab without creating an OS browser window', async () => {
    const h = await worker([{ id: firstId, conversationId: null }]);
    await h.maintain();
  ...
  it('keeps catalog discovery in an inactive internal tab even when foreground chats are preferred', async () => {
    const h = await worker([]);
  ...
  it('places an offered worker in an inactive internal tab with discard protection', async () => {
    const h = await worker([]);
    let offered = true;
  ...
  it('binds the reserved opening before route promotion releases provisional history', async () => {
    ...
    expect(h.create).not.toHaveBeenCalled();
    expect(h.sendMessage.mock.calls.every(([, message]) => message.type === 'clf-recorder-ping')).toBe(true);
  });
  ...
  it('reserves initial catalog opening before the internal host acts and never retries an ambiguous failure', async () => {
    const h = await worker([]);
    ...
    h.create.mockImplementation(async () => {
      ...
      throw new Error('Internal host may already have created the tab');
    });
    await h.inspectModels(request, true);
    await h.inspectModels(request, true);
    expect(h.create).toHaveBeenCalledTimes(1);
  ...
  it('creates background chats in the shared internal host without any OS-window ownership', async () => {
    const h = await worker([]);
    ...
    expect(h.create.mock.calls).toHaveLength(2);
    expect(h.create.mock.calls.every(([args]) => args.active === false && args.windowId === undefined)).toBe(true);
    expect(h.windows.create).not.toHaveBeenCalled();
    expect(h.windows.update).not.toHaveBeenCalled();
  ...
  it('releases a failed flight without granting another opening after ambiguous internal-host failure', async () => {
    const h = await worker([{ id: firstId, conversationId: null }]);
    h.tabs.push({ id: 50, url: `https://chatgpt.com/?other=cos-input=${firstId}` });
    h.create.mockRejectedValueOnce(new Error('Internal host temporarily refused tab creation'));
...
describe('Stop owns one exact existing or newly opened browser document', () => {
  ...
  it.each(['loading', 'unregistered'])('elects an existing %s exact tab instead of opening another', async mode => {
    ...
    expect(h.create).not.toHaveBeenCalled();
    expect(h.sendMessage.mock.calls.some(([, message]: [number, any, any?]) => message.type === 'clf-stop-turn')).toBe(false);
  ...
  it.each(['closed', 'navigated', 'lost-create-reply'])('retains spent opening custody across suspension after %s', async mode => {
    ...
    expect(restarted.create).not.toHaveBeenCalled();
    expect(restarted.sendMessage.mock.calls.some(([, message]: [number, any, any?]) => message.type === 'clf-stop-turn')).toBe(false);
  });
  it('does not treat query failure or an expired command as permission to open', async () => {
    const h = await worker([]) as any;
    // The bridge call itself may retry after rediscovery. Keep the host unavailable across that
    // bounded retry so this still proves that absence of a trustworthy tab scan grants no opener.
    h.query.mockRejectedValue(new Error('Internal browser host unavailable'));
