...
it('projects Browser and Pets, current checks and commands through the narrow menu preload', async () => {
  ...
  expect(buttons.map(button => button.dataset.command))
    .toEqual(['browser', 'pet', 'sidebar', 'zoom-in', 'zoom-out', 'zoom-reset']);
  expect(dom.window.document.querySelector('#viewBrowser span')!.textContent).toBe('ChatGPT browser');
  expect(dom.window.document.querySelector('#viewBrowser .chatgpt-mark')).not.toBeNull();
  ...
  expect(dom.window.document.querySelectorAll('.menu-check.ph-check')).toHaveLength(3);
  expect(dom.window.document.querySelectorAll('.view-menu-surface svg')).toHaveLength(1);
  expect(dom.window.document.getElementById('viewBrowser')!.getAttribute('aria-pressed')).toBe('true');
