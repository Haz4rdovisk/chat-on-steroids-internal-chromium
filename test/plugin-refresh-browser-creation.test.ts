...
it('keeps one operation on its original tab after marker loss, worker restart, and user closure', async () => {
  ...
  const start = () => {
    const tabs = { query: async () => tab ? [tab] : [], get: async () => { if (!tab) throw Error('closed'); return tab; }, update, remove: vi.fn() };
    const context = vm.createContext({ URL, setTimeout, clearTimeout, CHATGPT_TAB_URLS: ['https://chatgpt.com/*'], createChatTab: create, browserTabs: tabs,
      call: async () => ({ ok: true, data: { requests: [{ id, appId: null }] } }),
      chrome: { storage, tabs: { ...tabs, sendMessage: async () => ({ ok: true }) } } });
...
it('does not create a plugin helper in browser-only mode', async () => {
  ...
  const context = vm.createContext({ URL, setTimeout, clearTimeout, CHATGPT_TAB_URLS: ['https://chatgpt.com/*'], createChatTab: create, browserTabs: tabs,
    ...
    chrome: { storage: { session: { get: async () => ({}) } }, tabs } });
...
it('records browser creation failure before claim and retries the same obligation', async () => {
  ...
  const tabs = { query: async () => [] };
  const context = vm.createContext({ call, createChatTab, URL, setTimeout, clearTimeout, browserTabs: tabs,
    CHATGPT_TAB_URLS: ['https://chatgpt.com/*'], chrome: { storage: { session: { get: async () => ({}), set: async () => {} } }, tabs } });
  vm.runInContext(`${workflow}\nglobalThis.run = inspectRequestedPluginRefresh;`, context);
