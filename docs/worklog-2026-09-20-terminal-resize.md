...
- `npm run build`
- The unrestricted full run reached 5,608 passed / 44 skipped and exposed one existing
  PowerShell timing case under process contention. Its exact case passed alone, and its complete
  `mcp.test.ts` file passed 171/171.
- The complete suite then passed with bounded concurrency: 5,609 passed / 44 skipped. The
  separately owned shutdown suite passed 6/6.
- Impeccable detector found no issue in the changed terminal selectors. Its sole warning was an
  unrelated pre-existing File panel border in the Mainstream stylesheet.
